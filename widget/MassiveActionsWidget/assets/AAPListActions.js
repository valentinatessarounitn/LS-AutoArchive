...
var onClickListAction = function () {
    ...
    $modalButton.off('click').on('click', function () {
        ...
        if (curCKEDITOR && curCKEDITOR.instances && curCKEDITOR.instances['sEmailBody']) {
            // CKEDITOR è caricato e l'istanza 'sEmailBody' esiste
            // PRE textarea deve avere come ID sEmailBody
            $postDatas['sEmailBody'] = curCKEDITOR.instances['sEmailBody'].getData();

        } else {
            // CKEDITOR is NOT loaded: use the default textarea
            $postDatas['sEmailBody'] = $('textarea[name="sEmailBody"]').val();
        }


        $postDatas['sEmailSubject'] = $('input[name="sEmailSubject"]').val();

        // Necessary to differentiate between a message for future expiration and deactivation
        $postDatas['sOperationType'] = $('input[name="sOperationType"]').val();

        console.log('Contenuto di $postDatas: ', $postDatas);
