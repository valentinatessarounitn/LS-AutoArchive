...
class AAPSurvey extends Survey
{
    ...
    public function search($options = [])
    {
        ...
        if (isset($this->withoutResponse) && $this->withoutResponse) {
            ...
            // TODO: Implementare il filtro per i sondaggi senza risposte
        ...
        if (isset($this->withResponse) && $this->withResponse) {
            ...
            // TODO: Implementare il filtro per i sondaggi con risposte
    ...
    private function getSurveyIdsWithActivationDateOlderThan(int $months): array
    {
        ...
        foreach ($allSurveys as $survey) {
            Yii::log("Activation date for survey ID: $survey->sid", CLogger::LEVEL_WARNING);
            $activationDate = AAPDatabaseHelper::getSurveyActivationDate($survey->sid);

            Yii::log("Activation date ottenuta: $activationDate", CLogger::LEVEL_WARNING);
            if ($activationDate) {
    ...
    private function getSurveyIdsWithDeactivationDateOlderThan(int $months): array
    {
        ...
        foreach ($allSurveys as $survey) {
            ...
            // $deactivationDate: data di creazione della tabella old_survey_[ID] o, se mancante, dal contenuto della tabella AutoArchive
